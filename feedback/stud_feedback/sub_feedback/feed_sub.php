...
<?php else:
    $branch = $user['branch'];
    $sem = $user['sem'];
    $res = $conn->query("SELECT `subject_code`,`subject_name` FROM `subject_teacher_info` WHERE `branches_taught` LIKE '%$branch%' AND `sem`='$sem' AND `deg`='".$user['degree']."' ");
    include('common.php');
?>
  ...
  $(document).ready(function(){
    $('#sub').change(function(){
        ...
        $(this).removeAttr('disabled');
            ...
            $.ajax({
            type:"POST",
            url:"sub_feed_insert.php",
            data:$('#feed_form').serialize(),
            success:function(data){
               
                 alert("Feedback submitted successfully!!");
                 var url = 'http://localhost/stud_feed_sys/feedback/stud_feedback/sub_feedback/feed_sub.php';
