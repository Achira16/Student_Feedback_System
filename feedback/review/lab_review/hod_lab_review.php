...
<?php include('/opt/lampp/htdocs/stud_feed_sys/profiles/rev_profiles/rev_sidenav.php');?>
    ...
    if(isset($_POST['submit'])):
      $arr = explode(" and ",$_POST['inst_name']);
      ...
      {
        array_push($pie,$conn->query("SELECT `$col_rows[0]`,COUNT(*) FROM `lab_feedback` WHERE `code`='".$_POST['sub']."' AND `inst_1`='$arr[0]' AND `inst_2`='$arr[1]' GROUP BY `$col_rows[0]`"));
      }
    ...
    $('#sub').change(function(){
                ...
                $.ajax({
                    ...
                    success:function(data){
                        ...
                        {
                          var $newOpt = $("<option>").attr("value",x[i]+" and "+x[i+1]).text(x[i]+" and "+x[i+1])
