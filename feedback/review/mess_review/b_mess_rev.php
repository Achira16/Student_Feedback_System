...
<?php include('/opt/lampp/htdocs/stud_feed_sys/profiles/rev_profiles/rev_sidenav.php');?>
    ...
    <?php
      ...
      {
        array_push($pie,$conn->query("SELECT `$col_rows[0]`,COUNT(*) FROM `mess_feedback` WHERE `gender`='Male' GROUP BY `$col_rows[0]`"));
      }
     ...
     ?>
       ...
       <?php
       $table = $conn->query("SELECT `attr7` FROM `mess_feedback` WHERE `gender` = 'Male'");
      include('pie.php');
    ?>  
