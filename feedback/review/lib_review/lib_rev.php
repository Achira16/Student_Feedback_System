...
<?php include('/opt/lampp/htdocs/stud_feed_sys/profiles/rev_profiles/rev_sidenav.php');?>
    ...
    <?php
      ...
      $cols = $conn->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'lib_feedback' AND COLUMN_NAME LIKE 'attr%'");
      $pie = array();
      while($col_rows = $cols->fetch_row())
      {
        array_push($pie,$conn->query("SELECT `$col_rows[0]`,COUNT(*) FROM `lib_feedback` GROUP BY `$col_rows[0]`"));
      }
      $cols1 = $conn->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'lib_feedback' AND COLUMN_NAME LIKE 'attr%'");
      $col_comm = $conn->query("SELECT COLUMN_COMMENT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'trial' AND TABLE_NAME = 'lib_feedback' AND COLUMN_COMMENT!=''");
     ?>
