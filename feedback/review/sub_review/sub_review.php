...
<?php include('/opt/lampp/htdocs/stud_feed_sys/profiles/rev_profiles/rev_sidenav.php');?>
    ...
    if(isset($_POST['submit'])):
      ...
      {
        array_push($pie,$conn->query("SELECT `$col_rows[0]`,COUNT(*) FROM `sub_feedback` WHERE `subject_code`='".$_POST['sub']."' AND `instructor`='".$user['name']."' GROUP BY `$col_rows[0]`"));
      }
     ...
     ?>
      ...
      <?php 
      $table = $conn->query("SELECT `attr10` FROM `sub_feedback` WHERE `subject_code`='".$_POST['sub']."' AND `instructor`='".$user['name']."' AND `attr10`!=''");
      include('pie.php');
    ?>  
    
    <?php else:
    $res = $conn->query("SELECT distinct(`subject_code`),`subject_name` FROM `subject_teacher_info` WHERE `instructor`='".$user['name']."'");
    if(mysqli_num_rows($res)>0)
        include('common_r_fac.php');
    ?>
