...
<body>
   <?php include('/opt/lampp/htdocs/stud_feed_sys/profiles/rev_profiles/rev_sidenav.php');?>
    ...
    if(isset($_POST['submit'])):
      $cols = $conn->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'sub_feedback' AND COLUMN_NAME LIKE 'attr%'");
      $pie = array();
      while($col_rows = $cols->fetch_row())
      {
        array_push($pie,$conn->query("SELECT `$col_rows[0]`,COUNT(*) FROM `sub_feedback` WHERE `subject_code`='".$_POST['sub']."' AND `instructor`='".$_POST['inst_name']."' GROUP BY `$col_rows[0]`"));
      }
      $cols1 = $conn->query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'sub_feedback' AND COLUMN_NAME LIKE 'attr%'");
      $col_comm = $conn->query("SELECT COLUMN_COMMENT FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'trial' AND TABLE_NAME = 'sub_feedback' AND COLUMN_COMMENT!=''");
     ?>
      ...
      <?php 
      $table = $conn->query("SELECT `attr10` FROM `sub_feedback` WHERE `subject_code`='".$_POST['sub']."' AND `instructor`='".$_POST['inst_name']."' AND `attr10`!=''");
      include('pie.php');
    ?>  
    ...
    $res = $conn->query("SELECT distinct(`subject_code`),`subject_name` FROM `subject_teacher_info` WHERE `subject_code` LIKE '$branch%'");
    if(mysqli_num_rows($res)>0)
        include('common_r.php');
    ?>
     ...
     $('#sub').change(function(){
                ...
                $.ajax({
                    ...
                    success:function(data){
                        ...
                        $('#inst_name').empty();
                        $("#inst_name").append("<option value='' disabled selected>Choose instructor</option>");
                        for(var i=0;i<x.length;i++)
                        {
                          var $newOpt = $("<option>").attr("value",x[i]).text(x[i])
                          $("#inst_name").append($newOpt);
                        }
                       $('#inst_name').trigger('contentChanged');
                    }
                })
            });
     });
